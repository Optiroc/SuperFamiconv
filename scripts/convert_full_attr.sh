...
cargo run --release map -v --mode "$1" --in-image "$2" --in-attribute-map "$3" --in-palette out/palette.bin --in-tiles out/tiles.bin --out-data out/map.bin --out-json out/map.json && \
