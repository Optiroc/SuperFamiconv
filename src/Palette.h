...
#ifndef SFC_Palette_H
#define SFC_Palette_H
...
namespace sfc {
    ...
    struct Subpalette {
        ...
        sfc::Mode get_mode() const { return _mode; }

        const std::vector<rgba_t> get_normalized_colors() const {
            return sfc::normalize_colors(_colors, _mode);
        }
        ...
        void pad() { while (_colors.size() < _max_colors) add(0, true); }
