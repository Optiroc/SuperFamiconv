...
impl Palette {
    ...
    /// Renders a preview image of the palette, one subpalette per row.
    pub fn preview(&self) -> Result<Image, String> {
        let rows = self.normalized_colors();
        if rows.first().is_none_or(Vec::is_empty) {
            return Err("No colors".into());
        }

        let width = self.max_colors_per_subpalette as u32;
        let height = rows.len() as u32;
        let mut data = vec![NormalizedColor::TRANSPARENT; (width * height) as usize];
        for (y, row) in rows.iter().enumerate() {
            for (x, &c) in row.iter().enumerate() {
                data[y * width as usize + x] = c;
            }
        }
...
mod tests {
    ...
    fn image_from_colors(data: &[NormalizedColor]) -> Image {
        Image::from_color_data(data.len() as u32, 1, data.to_vec())
    }
