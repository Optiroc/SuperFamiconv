...
impl Map {
    ...
    /// Renders a preview image of the map using its corresponding `tileset` and `palette`.
    pub fn preview(
        &self,
        tileset: &Tileset,
        palette: &Palette,
        ...
        let width = self.width * tile_width;
        let height = self.height * tile_height;
...
mod tests {
    ...
    fn mock_image(colors: &[NormalizedColor; 8]) -> Image {
        let pixels: Vec<NormalizedColor> = colors.repeat(8);
        Image::from_color_data(8, 8, pixels)
    }
