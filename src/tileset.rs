...
impl Tileset {
    ...
    /// Renders a preview image of the tileset, tiles laid out row-major at `width` px wide.
    pub fn preview(
        &self,
        width: Option<u32>,
    ) -> Result<Image, String> {
        let width = width.unwrap_or(128).max(1);
        let tiles = self.tiles();
        let first = tiles.first().ok_or("Empty tileset")?;

        let (tile_width, tile_height) = (self.tile_width(), self.tile_height());
        ...
        for (i, tile) in tiles.iter().enumerate() {
            ...
            for row in 0..tile_height {
                for col in 0..tile_width {
                    ...
                    data[dst] = tile_rgba[src];
                    indexed_data[dst] = tile.data()[src];
                }
            }
        }
