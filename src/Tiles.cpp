...
namespace sfc {
    ...
    Tile::Tile(const Image& image, sfc::Mode mode, unsigned bpp, bool no_flip) {
        ...
        index_t mask = sfc::bitmask_at_bpp(_bpp);
        for (index_t ip : image.indexed_data()) _data.push_back(ip & mask);

        if (!no_flip) {
            _mirrors.push_back(sfc::mirror(_data, _width, true,  false));
            _mirrors.push_back(sfc::mirror(_data, _width, false, true));
            _mirrors.push_back(sfc::mirror(_data, _width, true,  true));
        }
    }

    Tile::Tile(const std::vector<uint8_t>& native_data, sfc::Mode mode, unsigned bpp, bool no_flip, unsigned width, unsigned height) {
        _mode = mode;
        _bpp = bpp;
        _width = width;
        _height = height;
        _data = sfc::unpack_native_tile(native_data, mode, bpp, width, height);

        _palette.resize(sfc::palette_size_at_bpp(bpp));
        ...
        for (int my = 0; my < metatiles_v; ++my) {
            for (int mx = 0; mx < metatiles_h; ++mx) {
                for (int blit_y = 0; blit_y < metatile_dim; ++blit_y) {
                    std::memcpy(&_data[((blit_y + (my * metatile_dim)) * width) + (mx * metatile_dim)], &metatile[metatile_index]._data[blit_y * metatile_dim], metatile_dim);
                }
                ++metatile_index;
            }
        }
    ...
    bool Tile::operator==(const Tile& other) const {
        ...
        if (!_mirrors.empty()) {
            for (auto& m : _mirrors) if (other._data == m) return true;
        }
        return false;
    }
    ...
    Tile Tile::crop(unsigned x, unsigned y, unsigned crop_width, unsigned crop_height) const {
        ...
        if (x > _width || y > _height) {
            ...
            for (int iy = 0; iy < blit_height; ++iy) std::memcpy(&t._data[iy * t._width], &_data[(x) + ((iy + y) * _width)], blit_width);
        }

        if (_mirrors.size()) {
            t._mirrors.push_back(sfc::mirror(t._data, t._width, true,  false));
            t._mirrors.push_back(sfc::mirror(t._data, t._width, false, true));
            t._mirrors.push_back(sfc::mirror(t._data, t._width, true,  true));
        }
    ...
    std::vector<uint8_t> Tile::native_data() const {
        return sfc::pack_native_tile(_data, _mode, _bpp, _width, _height);
    }

    std::vector<rgba_t> Tile::rgba_data() const {
        std::vector<rgba_t> v(_data.size());
        for (unsigned i = 0; i < _data.size(); ++i) v[i] = (_data[i] == 0) ? transparent_color : _palette[_data[i]];
        return v;
    }


    Tileset::Tileset(const std::vector<uint8_t> native_data, sfc::Mode mode, unsigned bpp, unsigned tile_width, unsigned tile_height, bool no_flip) {
        _mode = mode;
        _bpp = bpp;
        ...
        if (_mode == Mode::snes || _mode == Mode::snes_mode7) {
            ...
            if (native_data.size() % bytes_per_tile != 0) throw std::runtime_error("Tile data can't be deserialized (size doesn't match bpp setting)");
            unsigned tiles = (unsigned)native_data.size() / bytes_per_tile;

            for (unsigned i = 0; i < tiles; ++i) {
                _tiles.push_back(Tile(std::vector<uint8_t>(&native_data[i * bytes_per_tile], &native_data[(i + 1) * bytes_per_tile]), mode, bpp, no_flip, 8, 8));
            }
        }
    ...
    void Tileset::add(const sfc::Image& image, const Palette* palette) {
        Tile tile;
    ...
    int Tileset::index_of(const sfc::Tile& tile) const {
    ...
    void Tileset::save(const std::string& path) const {
        sfc::write_file(path, native_data());
    }
    ...
    std::vector<Tile> Tileset::remap_tiles_for_output(const std::vector<Tile>& tiles, Mode mode) const {
        ...
        if (mode == Mode::snes) {
            ...
            for (int i = 0; i < _tiles.size(); ++i) {
                unsigned base_pos = (((i / tiles_per_row) * cells_per_tile_v) * cells_per_row) + ((i % tiles_per_row) << (cells_per_tile_h - 1));
    ...
    std::vector<Tile> Tileset::remap_tiles_for_input(const std::vector<Tile>& tiles, Mode mode) const {
        ...
        if (mode == Mode::snes) {
            ...
            for (int i = 0; i < _tiles.size(); ++i) {
                ...
                if (metatile.size() == cells_per_tile_h * cells_per_tile_v) tv.push_back(Tile(metatile, _no_flip, _tile_width, _tile_height));
            }
