...
namespace sfc {
    ...
    void Map::add(const sfc::Image& image, const sfc::Tileset& tileset, const sfc::Palette& palette, unsigned bpp, unsigned pos_x, unsigned pos_y) {
        if (((pos_y * _map_width) + pos_x) > _entries.size()) throw std::runtime_error("Adding map entry out of bounds");
    ...
    const std::string Map::to_json(bool column_order, unsigned split_w, unsigned split_h) const {
        ...
        for (auto& vm : vmm) {
            ...
            for (auto& m : vm) ja.push_back({{"tile", m.tile_index}, {"palette", m.palette_index}, {"flip_h", (int)m.flip_h}, {"flip_v", (int)m.flip_v}});
            if (vmm.size() > 1) {
