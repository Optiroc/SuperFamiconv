...
namespace sfc {
    ...
    struct Tile {
        Tile(const Image& image, sfc::Mode mode = Mode::snes, unsigned bpp = 4, bool no_flip = false);
        Tile(const std::vector<uint8_t>& native_data, sfc::Mode mode = Mode::snes, unsigned bpp = 4, bool no_flip = false, unsigned width = 8, unsigned height = 8);
        Tile(const std::vector<Tile>& metatile, bool no_flip, unsigned width, unsigned height);

        Tile(sfc::Mode mode, unsigned bpp, unsigned width, unsigned height)
        : _mode(mode), _bpp(bpp), _width(width), _height(height) {
    ...
    struct Tileset {
        ...
        : _mode(mode), _bpp(bpp), _tile_width(tile_width), _tile_height(tile_height),
          _no_discard(no_discard), _no_flip(no_flip), _no_remap(no_remap), _max_tiles(max_tiles) {};

        Tileset(const std::vector<uint8_t> native_data, sfc::Mode mode = Mode::snes, unsigned bpp = 4, unsigned tile_width = 8, unsigned tile_height = 8, bool no_flip = false);
