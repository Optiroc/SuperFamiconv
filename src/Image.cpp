...
namespace sfc {
    ...
    Image::Image(const std::string& path) {
        ...
        if (state.info_raw.colortype == LCT_PALETTE) {
            ...
            for (int i = 0; i < state.info_raw.palettesize * 4; i += 4) {
                uint32_t color = (state.info_raw.palette[i]) + (state.info_raw.palette[i+1] << 8) + (state.info_raw.palette[i+2] << 16) + (state.info_raw.palette[i+3] << 24);
        ...
        if (state.info_png.color.colortype == LCT_RGB || state.info_png.color.colortype == LCT_GREY || state.info_png.color.colortype == LCT_GREY_ALPHA) {
            state.info_raw.colortype = LCT_RGBA;
            needs_conversion = true;
        }
    ...
    Image::Image(const Image& image, const sfc::Subpalette& subpalette) {
        ...
        sfc::Mode mode = subpalette.get_mode();
        _width = image.width();
    ...
    // nb! set_pixel & blit won't touch indexed data
