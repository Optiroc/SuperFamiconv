...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        sfc::Palette palette;
        sfc::Tileset tileset(settings.mode, settings.bpp, settings.tile_w, settings.tile_h, settings.no_discard, settings.no_flip, settings.no_remap, settings.max_tiles);
        ...
        if (verbose) {
            if (settings.no_discard) {
                ...
                std::cout << "Created optimized tileset with " << tileset.size() << " tiles (discarded " << tileset.discarded_tiles << " redudant tiles)\n";
            }
        }
