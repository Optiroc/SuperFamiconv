...
namespace sfc {
    ...
    void Palette::add(const std::vector<rgba_t>& colors) {
        auto rc = sfc::reduce_colors(colors, _mode);
    ...
    void Palette::add_noremap(const std::vector<rgba_t>& colors, bool reduce) {
        auto rc = colors;
        if (reduce) rc = sfc::reduce_colors(rc, _mode);
        auto splits = sfc::split_vector(rc, _max_colors_per_subpalette);
        for (auto& sv : splits) {
    ...
    const Subpalette& Palette::subpalette_matching(const Image& image) const {
        auto rc = sfc::reduce_colors(image.rgba_data(), _mode);
        std::set<rgba_t> cs(rc.begin(), rc.end());
        cs.erase(transparent_color);
        if (cs.size() > _max_colors_per_subpalette) throw std::runtime_error("Colors don't fit in palette"); // TODO: catch and report position
        ...
        for (const Subpalette& sp : _subpalettes) {
            if (sp.diff(cs) == 0) { matching_sp = &sp; break; }
        }
    ...
    const std::vector<std::vector<rgba_t>> Palette::normalized_colors() const {
        auto v = colors();
        for (auto& i : v) i = sfc::normalize_colors(i, _mode);
        return v;
    }
    ...
    Palette::Palette(const std::vector<uint8_t> native_data, Mode in_mode, unsigned colors_per_subpalette) {
        ...
        _max_subpalettes = 64;
        add_noremap(sfc::unpack_native_colors(native_data, in_mode), false);
    }
    ...
    Palette::Palette(const std::string& path, Mode in_mode, uint32_t colors_per_subpalette) {
        ...
        try {
            // load json
            auto j = sfc::read_json_file(path);
            auto jp = j["palettes"];
            for (auto jsp : jp) {
                std::vector<rgba_t> colors;
                for (auto jcs : jsp) if (jcs.is_string()) colors.push_back(sfc::reduce_color(sfc::from_hexstring(jcs), in_mode));
            ...
            // load binary
            add_noremap(sfc::unpack_native_colors(sfc::read_binary(path), in_mode), false);
        }
    ...
    const std::string Palette::to_json() const {
        ...
        for (auto i : v) {
            ...
            for (auto j : i) vs.push_back(sfc::to_hexstring(j));
            vj.push_back(vs);
        }
    ...
    void Palette::save(const std::string& path) const {
        ...
        for (auto& sp : _subpalettes) {
            ...
            for (auto c : colors) {
                auto nc = sfc::pack_native_color(c, _mode);
                data.insert(data.end(), nc.begin(), nc.end());
            }
        }

        sfc::write_file(path, data);
    }
